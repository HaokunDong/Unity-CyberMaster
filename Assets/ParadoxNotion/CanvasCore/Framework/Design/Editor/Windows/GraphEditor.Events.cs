        ...
        static void HandlePostNodesGraphEvents(Graph graph, Vector2 canvasMousePos) {
            ...
            if ( GraphEditorUtility.allowClick ) {
                ...
                if ( isContext || isShortcut ) {
                    if (AssetTracker.trackedTypes == null || !AssetTracker.trackedTypes.Contains(typeof(Macro)))
                    {
                        AssetTracker.BeginTrackingAssetsOfType(typeof(Macro));
                    }
                   ...
                   //     }
                   // }
                   //  GenericMenuBrowser.Show(menu, e.mousePosition, "Add Node", graph.baseNodeType);
                    e.Use();
                }
            }
        }
