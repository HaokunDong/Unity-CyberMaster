...
public class FluidController : SingletonComp<FluidController>
{
    ...
    void Update()
    {
        ...
        {
            followObject = World.Ins?.Player?.gameObject;
            return;
        }

        if (obstacleCamera == null)
        {
            obstacleCamera = GameObject.Find("Sub Camera")?.GetComponent<Camera>();
            return;
        }
        else
        {
            obstacleCamera.transform.position = new Vector3(transform.position.x, transform.position.y, -10);
        }
